...
void sfx_join_sv_graph_nodes( vector<sv_graph_node*> *nodes, int threshold ) {
	...
	vector <u_nm_pair*> *run1, *run2, *run3, *run4;

	sfx_join_nodes( u_nm_pairs, threshold, &run1);

	cout << run1->size() << "\t";
	sfx_join_nodes( run1, threshold, &run2);

	cout << run2->size() << "\t";
	sfx_join_nodes( run2, threshold, &run3);

	cout << run3->size() << "\t";
	sfx_join_nodes( run3, threshold, &run4);
...
void sfx_join_nodes( vector<u_nm_pair*> *nodes, int threshold, vector<u_nm_pair*> **result) {
	...
	for (group_iter = groups->begin(); group_iter != groups->end();
		...
		//vector< pair<int, u_nm_pair>* > *composites = 
				//new vector< pair<int, u_nm_pair>* >();
...
int main(int argc,char** argv) {
	...
	while ( i != nm_pairs->end() ) {
		...
		/*
		if (nodes->size() > 200)
			sfx_join_sv_graph_nodes( nodes, sfx_T);
		*/

		sfx_connect_nodes( nodes, sfx_T);

		for (j = nodes->begin(); j != nodes->end(); j++) {
			sv_graph_component *component = new sv_graph_component();
			sv_graph_node *v = (*j)->get_pair();
	...
	cerr << "Finding SCS." << endl;

	/*
	 * Within each connected component, find the shortest common superstring
	 */
	vector< sv_graph_component* >::const_iterator k;
	for (k = components->begin(); k != components->end(); k++) {
		vector<sv_graph_node*>::const_iterator l;
		// Do a greedy search from each node in the componenet
		int component_max_w = -1;
		string component_max_w_string = "";
		for (	l = (*k)->get_nodes()->begin(); 
				l != (*k)->get_nodes()->end(); 
				l++) {
			int w = 0;
			sv_graph_node *v = *l;
			...
			if (w > component_max_w) {
				component_max_w = w;
				component_max_w_string = contig;
			}
		}
		(*k)->set_contig(component_max_w_string);

		/* Print properties of each component 
		cout << (*k)->get_min_node()->get_pair()->get_offset() << "\t" <<
				(*k)->get_max_node()->get_pair()->get_offset() << "\t" <<
				(*k)->size() << "\t" <<
				(*k)->get_contig() << endl;
		*/
	}

	vector< pair<sv_graph_component*, sv_graph_component*> > *pairs =
			new vector< pair<sv_graph_component*, sv_graph_component*> >();

	sort(components->begin(),  components->end(), 
			sv_graph_component::compare_sv_graph_components);

	vector< sv_graph_component* >::const_iterator start, stop;
	...
	for (k = components->begin(); k != components->end(); k++) {
		sfx_tree k_tree( (*k)->get_contig() );

		//Adjust window
		while (	(start != components->end()) && 
				( (*k)->get_min_node()->get_pair()->get_offset() -
				  (*start)->get_max_node()->get_pair()->get_offset() > 
				  component_neighbor) )
			start++;

		while (	(stop != components->end()) && 
				( (*stop)->get_min_node()->get_pair()->get_offset() - 
				  (*k)->get_max_node()->get_pair()->get_offset() <
				  component_neighbor) )
			stop++;

		vector< sv_graph_component* >::const_iterator l;
		for (l = start; l != stop; l++) {
			sfx_tree l_tree( rev_comp( (*l)->get_contig() ) );

			int max_sfx_f = k_tree.overlap( rev_comp( (*l)->get_contig() ) );
			int max_sfx_r = l_tree.overlap( (*k)->get_contig() );
			...
			double a = (double)max_sfx / 
					   (double)( (*k)->get_contig().length() );
			double b = (double)max_sfx / 
					   (double)( (*l)->get_contig().length() );


			if ( (a > 0.5) || (b > 0.5) )
				pairs->push_back(
						pair<sv_graph_component*, sv_graph_component*> (
						(*k), (*l)));
			...
			/*  Print out the comparisons
				cout << 
					(*k)->get_min_node()->get_pair()->get_offset() << "\t" <<
					(*k)->get_max_node()->get_pair()->get_offset() << "\t" <<
					(*l)->get_min_node()->get_pair()->get_offset() << "\t" <<
					(*l)->get_max_node()->get_pair()->get_offset() << "\t" <<
					max_sfx_f << "\t" <<
					max_sfx_r << "\t" <<
					(*k)->get_contig() << "\t" <<
					rev_comp( (*l)->get_contig() ) << "\t" << 
					a << "\t" <<
					b << "\t" <<
					endl;
			*/
	...
	vector< pair<sv_graph_component*, sv_graph_component*> >::const_iterator l;
	for(l = pairs->begin(); l != pairs->end(); l++) {
		sv_graph_component *a, *b;
		a = (*l).first;
		b = (*l).second;

		vector<int> x;
		x.push_back(a->get_max_node()->get_pair()->get_offset());
		x.push_back(a->get_min_node()->get_pair()->get_offset());
		x.push_back(b->get_max_node()->get_pair()->get_offset());
		x.push_back(b->get_min_node()->get_pair()->get_offset());
		int min,max;
